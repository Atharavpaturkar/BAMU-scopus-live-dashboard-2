...
/**
 * app.js - Dr. Babasaheb Ambedkar Marathwada University (BAMU) Live Scopus Intelligence Dashboard
...
(function () {
  ...
  function renderMonthlyVelocity(filtered, year) {
    ...
    const maxMonthVal = Math.max(...monthCounts, 5);
    const monthTrace = {
      x: months,
      y: monthCounts,
      type: "bar",
    ...
    const monthLayout = applyChartTheme({
      ...
      xaxis: { title: "Month" },
      yaxis: { title: "Publications", range: [0, Math.ceil(maxMonthVal * 1.25)] }
    });
  ...
  function renderTabImpact(filtered) {
    ...
    const maxDeptCite = Math.max(...deptVals, 10);
    const deptBarTrace = {
      x: deptVals,
      y: deptLabels,
      type: "bar",
    ...
    const deptBarLayout = applyChartTheme({
      margin: { l: 150, r: 90, t: 25, b: 35 },
      xaxis: { title: "Cumulative Citations", range: [0, Math.ceil(maxDeptCite * 1.25)] },
  ...
  function renderTabCollab(filtered) {
    ...
    const maxCountryCount = Math.max(...top10.map(c => c.count), 10);
    const topBar = [{
      type: "bar",
      orientation: "h",
    ...
    const topBarLayout = applyChartTheme({
      margin: { l: 110, r: 90, t: 15, b: 35 },
      xaxis: { title: "Joint Publications", range: [0, Math.ceil(maxCountryCount * 1.25)] }
    });
    ...
    const labels = ["Dr. Babasaheb Ambedkar Marathwada University"];
    const parents = [""];
    ...
    uniqueDepts.forEach(dept => {
      labels.push(dept);
      parents.push("Dr. Babasaheb Ambedkar Marathwada University");
    ...
    const maxIndPct = Math.max(...indList.map(i => i.pct), 10);
    const indTrace = [{
      type: "bar",
      orientation: "h",
    ...
    const indLayout = applyChartTheme({
      margin: { l: 120, r: 90, t: 15, b: 35 },
      xaxis: { title: "Corporate / Industry Collaboration (%)", range: [0, Math.min(100, Math.ceil(maxIndPct * 1.25))] }
    });
  ...
  function renderAuthorDeepDive(filtered, authorName) {
    ...
    const primaryDept = authPubs[0] ? authPubs[0].department : "Dr. Babasaheb Ambedkar Marathwada University";

    // Update Header Card
    document.getElementById("auth-header-name").textContent = authorName;
    document.getElementById("auth-header-dept").textContent = `🏛️ ${primaryDept} • Dr. Babasaheb Ambedkar Marathwada University`;
  ...
  function initAICopilot() {
    if (state.chatMessages.length === 0) {
      state.chatMessages.push({
        role: "assistant",
        content: `### 👋 Dr. Babasaheb Ambedkar Marathwada University Research AI Copilot
...
* *"Who are the top cited faculty members?"*`
  ...
  function generateAIResponse(query, filtered) {
    ...
    if (q.includes("dossier") || q.includes("overview") || q.includes("executive") || q.includes("summary")) {
      return `### 📊 Executive Bibliometric Dossier: Dr. Babasaheb Ambedkar Marathwada University (BAMU)

**Institutional Affiliation**: Dr. Babasaheb Ambedkar Marathwada University (\`AF-ID: 60028245\`)  
...
2. **Global Collaboration**: Cross-border international co-authorship at **${kpis.international_collab_pct}%** establishes robust presence across Europe, North America, and Asia.`;
    ...
    if (q.includes("dept") || q.includes("department") || q.includes("ranking") || q.includes("rankings")) {
      ...
      let table = `### 🏛️ Dr. Babasaheb Ambedkar Marathwada University Department Research Leaderboard\n\n| Rank | Academic Department | Publications | Citations | CPP | Q1 Share |\n| :--- | :--- | :--- | :--- | :--- | :--- |\n`;
...
* **Accreditation Impact**: High Q1 concentration is critical for NAAC Criterion 3 and NIRF Research & Professional Practice parameters.`;
    ...
    if (q.includes("author") || q.includes("faculty") || q.includes("researcher") || q.includes("laureate") || q.includes("who")) {
      const leaderboard = getAuthorLeaderboard(filtered);
      let res = `### 👥 Leading Faculty Researchers at Dr. Babasaheb Ambedkar Marathwada University\n\n| Rank | Faculty Member | Department | Publications | Citations | CPP | h-Index |\n| :--- | :--- | :--- | :--- | :--- | :--- | :--- |\n`;
...
*Tip: Use the analytical shortcuts above or query specific departments (e.g. Chemistry, Physics, Life Sciences) or faculty members.*`;
  ...
  // ---------------------------------------------------------
  function exportToExcel(records, filename = "bamu_scopus_report.xlsx") {
    if (!window.XLSX) {
  ...
  function exportToBibTeX(records, filename = "bamu_scopus.bib") {
  ...
  function printAuthorProfile(authorName, filtered) {
    ...
    const cpp = totalPapers > 0 ? (totalCitations / totalPapers).toFixed(2) : "0.00";
    const hIndex = calculateHIndex(authPubs.map(d => Number(d.citations) || 0));
    const primaryDept = authPubs[0].department || "Dr. Babasaheb Ambedkar Marathwada University";
    ...
    const printHtml = `
      ...
      <body>
        <div class="header">
          ...
          <div class="dept">🏛️ ${primaryDept} • Dr. Babasaheb Ambedkar Marathwada University (IR-O-U-0318)</div>
        </div>
        <div class="kpi-row">
